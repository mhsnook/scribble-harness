...
//
// `parse` reads the JSON shape Vitest emits under `--reporter=json`, which is
// the Jest shape. All four projects — shared, client, worker and stories —
// report into the one file, so a failure names its own test rather than its
// project.
