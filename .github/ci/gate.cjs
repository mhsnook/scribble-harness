...
//
// The repo is clean today, so `no-new` costs nothing to adopt and there is no
// debt to ratchet down. The bundle starts report-only: nobody has watched these
// numbers across a few PRs yet, and a budget picked before that is a guess that
// blocks people.
const POLICY = {
	build: 'no-new',
	typecheck: 'no-new',
...
// job passes the gate.
//
// `scan` is in the list because `scan-build.sh` writes a `missing: true`
// sidecar rather than exiting early when there is no build to read, so its
// absence really does mean the step never ran. `build` is not: render-build.cjs
// deliberately writes no fragment when neither tree recorded an outcome.
const REQUIRED = ['typecheck', 'lint', 'format', 'tests', 'scan']
...
function verdict(check, data) {
	...
	if (check === 'scan') {
		return data.found > 0
			? `${data.found} forbidden pattern(s) in the build output`
			: null
	}
