...
// anchored on this rather than on the tail of the log, because `vite build`
// prints a chunk-size table, a plugin-timing block and a rolldown hint after
// the useful part — the last 40 lines are reliably the least informative 40.
//
// `error during build:` is Vite's own heading; `✘ [ERROR]` and `x [ERROR]` are
// what the Cloudflare plugin and rolldown print; `Could not resolve` is the
// common import failure, which Vite reports before its heading.
const ERROR_HEADING =
	/^\s*(error|ERROR|Error:|\[vite\]|✘|×|x \[ERROR\]|failed to|Could not resolve)/
...
// An ANSI escape IS a control character, so matching one is the whole point.
